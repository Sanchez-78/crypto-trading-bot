...
def detect_crashes(last_logs: List[str]) -> Tuple[bool, List[str]]:
    ...
    2026-08-26 v1 FIX: the old check flagged ANY log line containing the
    bare substring "Exception"/"ERROR"/"FATAL" as a crash -- this
    false-positived on already-handled, logged warnings whose message text
    happens to mention an exception class name (e.g. "WebSocket error:
    ...
    a CRITICAL CRASH_DETECTED alert.

    2026-08-26 v2 (after review): v1 additionally skipped any line
    containing this codebase's "⚠️" handled-warning marker. REJECTED on
    review: the positive-match narrowing above (Traceback/FATAL) already
    kills the production false positive on its own -- the emoji skip added
    no benefit but did cost real detection: `runtime_fault_registry.py`
    logs genuine CRITICAL faults as "⚠️  RUNTIME_FAULT [CRITICAL]: ..." and
    was being silently suppressed by the emoji check. Removed the emoji
    skip; added an explicit "RUNTIME_FAULT [CRITICAL]" marker instead, so
    that specific real-fault channel is still caught regardless of its ⚠️
    prefix. See _workspace/46_.../47_...md.
    """
    crash_lines = []
    ...
    for log_line in last_logs:
        if (
            ...
            or "RUNTIME_FAULT [CRITICAL]" in log_line
        ):
