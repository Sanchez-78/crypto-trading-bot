...
MIN_TP_PCT  = 0.0015    # V10.29: 0.15% min TP — architectural review: 0.40% unreachable in 180s hold window; achievable in 50% of trades per regime-adjusted ATR
...
def handle_signal(signal):
    ...
    # V10.27: Removed duplicate peak-check block from 4eaccf0. It referenced
    # _POSITION_LOCK/_POSITIONS/_is_position_stale, which live in
    # paper_trade_executor (not imported here) -> NameError on every entry.
    # The identical peak rejection already runs correctly upstream in
    # paper_trade_executor.open_paper_position() (commit 2d4aa48).
...
def on_price(data):
    ...
    if _is_paper_mode_local:
        ...
        # P1.1AA+V10.16: Check timeout only (TP/SL already handled by update_paper_positions above)
