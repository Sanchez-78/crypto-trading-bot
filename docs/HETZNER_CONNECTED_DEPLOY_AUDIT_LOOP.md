...
The deploy loop refuses to continue if `.env` contains:
...
sudo systemctl status cryptomaster-autodeploy.service --no-pager
sudo journalctl -u cryptomaster-autodeploy.service -n 200 --no-pager
```
