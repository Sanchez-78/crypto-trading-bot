...
| §20 Configuration | GAP | No startup validation of invalid flag combinations (§20's examples: `OFI_MODE=filter` while disabled, dynamic exit without ATR, real orders enabled, negative fee, max spread <=0) exists for the new pipeline. `.env.example` updated 2026-08-18 with the actual P0.8+ flags (real names, not the document's suggested ones — adapted per §29's own "adapt to the repository" guidance). |
